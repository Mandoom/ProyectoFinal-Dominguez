...
function NavBar ({pageTitle, bg, pad, color, brandfont }) { 
 return(
    <div className='navbar'style={{backgroundColor : bg, padding: pad, color : color}}>
        <div className='brand'>
        <div className="logo">
        <svg id="Layer_1" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 980 978.94" width="40" height="40" fill="currentColor">
        ...
        <nav>
           ...
           <ul className='menu'>
                <Link className={({ isActive }) => `(isActive ? 'active' : '') menu-item`} to='/'>Store</Link>
                <Link className="menu-link"to='/inventory'>Your Inventory</Link>
                <Link className="menu-link"to='/inventory'>Pokedex</Link>
